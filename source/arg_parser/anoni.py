...
def create_example_project(path: str):
    ...
    path += "example"  # Не создается копия при запуске из не arg_parser директории
...
def main():
    ...
    subparsers = parser.add_subparsers(
        dest="command", required=True, help="Команда для выполнения"
    )
    ...
    init_project_parser.add_argument(
        "project_path", help="Путь, по которому будет создан проект"
    )

    create_example = subparsers.add_parser(
        "create_example", help="Create project with examples"
    )
