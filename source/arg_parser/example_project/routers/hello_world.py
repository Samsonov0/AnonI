...
@hello_world_router.get('/hello_world/{name}')  # Use router.get("path") to create GET handler
...
@hello_world_router.get('/hello_world/{name}')  # Use {name} syntax to indicate expected data in url
async def say_hello_world_to_name(request_data: RequestData) -> DefaultScheme:
    ...
    '''
    name = request_data.query_params().get('name')
    
    data = json.dumps({
        'data': f'Hello World! And hello {name}'
    })

    success = DefaultScheme(
        status=HTTP_200_OK,
        body=data,
        headers=HEADERS
    )
